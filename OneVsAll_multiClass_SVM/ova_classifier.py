...
def classifiers(X_train, y_train):
    ...
    binary_classifiers = {}  # dictionary to store binary classifiers

    for class_label in unique_classes:
        binary_y_train = np.where(y_train == class_label, 1, 0)    
        svm_classifier = SVC(kernel='linear')
        svm_classifier.fit(X_train, binary_y_train)
        binary_classifiers[class_label] = svm_classifier
...
def prediction(X_generate, binary_classifiers):
    ...
    for i in range(X_generate.shape[0]):
        ...
        for class_label, classifier in binary_classifiers.items():
            decision_function_score = classifier.decision_function(X_generate[i:i+1])
            class_scores[class_label] = decision_function_score
...
plt.title('Confusion Matrix for OvA SVM')
plt.colorbar()
